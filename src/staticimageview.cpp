/*
    nanogui/staticimageview.cpp -- Widget used to display images.
...
#include <nanogui/staticimageview.h>
...
StaticImageView::StaticImageView(Widget* parent, GLuint imageID)
...
void StaticImageView::bindImage(GLuint imageId) {
    mImageID = imageId;
    updateImageParameters();
    fit();
}

Vector2f StaticImageView::imageCoordinateAt(const Vector2f& position) const {
    auto imagePosition = position - mOffset;
    return imagePosition / mScale;
}

Vector2f StaticImageView::clampedImageCoordinateAt(const Vector2f& position) const {
    auto imageCoordinate = imageCoordinateAt(position);
    return imageCoordinate.cwiseMax(Vector2f::Zero()).cwiseMin(imageSizeF());
}

Vector2f StaticImageView::positionForCoordinate(const Vector2f& imageCoordinate) const {
    return mScale*imageCoordinate + mOffset;
}

void StaticImageView::setImageCoordinateAt(const Vector2f& position, const Vector2f& imageCoordinate) {
...
void StaticImageView::center() {
    mOffset = (sizeF() - scaledImageSizeF()) / 2;
}

void StaticImageView::fit() {
    // Calculate the appropriate scaling factor.
    mScale = (sizeF().cwiseQuotient(imageSizeF())).minCoeff();
    center();
}

void StaticImageView::setScaleCentered(float scale) {
...
void StaticImageView::moveOffset(const Vector2f& delta) {
...
void StaticImageView::zoom(int amount, const Vector2f& focusPosition) {
...
bool StaticImageView::mouseDragEvent(const Vector2i& p, const Vector2i& rel, int button, int modifiers) {
...
bool StaticImageView::gridVisible() const {
    return (mGridThreshold != -1) && (mScale > mGridThreshold);
}

bool StaticImageView::pixelInfoVisible() const {
    return mPixelInfoCallback && (mPixelInfoThreshold != -1) && (mScale > mPixelInfoThreshold);
}

bool StaticImageView::helpersVisible() const {
    return gridVisible() || pixelInfoVisible();
}


bool StaticImageView::scrollEvent(const Vector2i& p, const Vector2f& rel) {
    if (mFixedScale)
        return false;
    float v = rel.y();
    if (std::abs(v) < 1)
...
bool StaticImageView::keyboardEvent(int key, int /*scancode*/, int action, int modifiers) {
    if (action) {
        switch (key) {
        case GLFW_KEY_LEFT:
            if (!mFixedOffset) {
...
bool StaticImageView::keyboardCharacterEvent(unsigned int codepoint) {
    switch (codepoint) {
    case '-':
        if (!mFixedScale) {
            zoom(-1, sizeF() / 2);
...
Vector2i StaticImageView::preferredSize(NVGcontext* /*ctx*/) const {
    return mImageSize;
}

void StaticImageView::performLayout(NVGcontext* ctx) {
    Widget::performLayout(ctx);
    center();
}

void StaticImageView::draw(NVGcontext* ctx) {
    Widget::draw(ctx);
...
void StaticImageView::updateImageParameters() {
...
void StaticImageView::drawWidgetBorder(NVGcontext* ctx) const {
    /*nvgBeginPath(ctx);
...
void StaticImageView::drawImageBorder(NVGcontext* ctx) const {
    /*nvgSave(ctx);
    nvgBeginPath(ctx);
...
void StaticImageView::drawHelpers(NVGcontext* ctx) const {
...
void StaticImageView::drawPixelGrid(NVGcontext* ctx, const Vector2f& upperLeftCorner,
...
void StaticImageView::drawPixelInfo(NVGcontext* ctx, float stride) const {
...
void StaticImageView::writePixelInfo(NVGcontext* ctx, const Vector2f& cellPosition,
